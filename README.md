...
Framework d'automatisation de tests de sécurité basé sur **Robot Framework**, conçu pour auditer automatiquement les fonctionnalités de sécurité critiques d'une application web : authentification, politique de mot de passe, contrôle d'accès (IAM), IDOR, API REST, headers/cookies de sécurité, et vulnérabilités OWASP Top 10. Le projet intègre également un scan dynamique (OWASP ZAP), une analyse de dépendances (npm audit) et un scan de vulnérabilités infrastructure (OpenVAS/Greenbone), le tout orchestré dans un pipeline CI/CD.

Testé en conditions réelles contre **[OWASP Juice Shop](https://github.com/juice-shop/juice-shop)** — plus de 35 tests automatisés, 5+ vulnérabilités réelles découvertes et documentées (voir plus bas).

## Objectif

Démontrer une approche **DevSecOps** : combiner tests fonctionnels de sécurité, DAST, analyse de dépendances et scan infrastructure, intégrés dans un pipeline CI/CD déclenché à chaque push, avec génération automatique de rapports consolidés.
...
│   ├── security_headers.robot     # Headers HTTP et cookies de sécurité
│   └── iam_advanced.robot         # Tests IDOR et privilege escalation
│
├── resources/
...
│
├── .github/workflows/
│   └── robot-tests.yml            # Pipeline CI/CD complet (Robot Framework + ZAP + npm audit)
│
├── requirements.txt
└── README.md
```
...
| `security_headers.robot` | ✅ 3/4 PASS | Headers de sécurité, cookies, fuite de mot de passe |
| `iam_advanced.robot` | ✅ 5/5 PASS | IDOR sur paniers, privilege escalation, historique de commandes |
...
| 4 | **Security Misconfiguration (headers manquants)** | A05:2021 | `GET /` | Absence des headers `Strict-Transport-Security` et `Content-Security-Policy`, confirmée à la fois par les tests Robot Framework et par un scan OWASP ZAP indépendant |
| 5 | **IDOR sur les paniers utilisateurs** | A01:2021 Broken Access Control | `GET /rest/basket/{id}` | Avec un seul compte, énumération des paniers d'ID 1 à 5 : accès systématique au contenu de paniers appartenant à des UserId différents (1, 2, 3, 11, 16), sans aucun contrôle d'autorisation |
| 6 | **Vulnérabilités de dépendances** | A06:2021 Vulnerable Components | `package.json` (backend & frontend) | `npm audit` révèle 24 vulnérabilités côté backend et 47 côté frontend (dont plusieurs critiques), notamment liées à des bibliothèques cryptographiques obsolètes |

**Méthodologie** : chaque faille a été découverte via les tests automatisés du framework lui-même, puis confirmée manuellement (requête API / inspection navigateur / scan ZAP) avant d'être formalisée en test reproductible.

## Prérequis

- Python 3.9+
- Google Chrome (le ChromeDriver est géré automatiquement par Selenium 4.25+)
- Docker (pour lancer OWASP Juice Shop et OpenVAS en local)
...
Le fichier `.github/workflows/robot-tests.yml` automatise, à chaque `push` ou `pull_request` sur `main`/`develop`, quatre jobs exécutés en parallèle :

1. **robot-tests** — Exécution de la suite complète de tests Robot Framework
2. **zap-scan** — Scan dynamique OWASP ZAP (Baseline Scan) sur Juice Shop
3. **dependency-scan** — Analyse des dépendances via `npm audit`
4. **consolidate-reports** — Agrégation de tous les rapports en un artifact unique téléchargeable

Tous les rapports sont publiés en artifacts téléchargeables depuis l'onglet **Actions** du repo GitHub.

## Scan de vulnérabilités infrastructure (OpenVAS / Greenbone)

En complément du scan applicatif (ZAP) et de l'analyse de dépendances (npm audit), un scan **OpenVAS/Greenbone Community Edition** a été intégré afin de se rapprocher d'une démarche de Vulnerability Management complète (proche de celle réalisée avec Qualys en entreprise), couvrant cette fois le niveau infrastructure plutôt qu'uniquement applicatif.

```bash
docker run -d -p 9392:9392 --name openvas -e PASSWORD=admin123 immauss/openvas
```

L'interface est ensuite accessible sur `https://localhost:9392` (identifiants : `admin` / mot de passe défini via la variable `PASSWORD`).
...
- Version obsolète de l'action GitHub `zaproxy/action-baseline` (v0.12.0) : échec d'upload d'artifact dû à un changement de l'API GitHub → mise à jour vers `v0.15.0`
- Code de sortie non nul de Robot Framework dès qu'un test échoue (ex : faille CSP détectée intentionnellement), faisant échouer le job CI/CD → ajout de `|| true` pour préserver la génération du rapport sans bloquer le pipeline
...
- Intégration de tests SAST (Semgrep) sur le code du framework lui-même
- Rapport de pentest structuré avec scoring CVSS

## Auteur

**Aya Sidki** — Étudiante en Cybersécurité & Cloud Computing, ENSAM Casablanca (Université Hassan II)

---

*Projet réalisé à des fins pédagogiques, dans le cadre d'un portfolio personnel en cybersécurité.*
